const app = require("express").Router(), Sentry = require("@sentry/node"), d3 = import("d3"), jsdom = require("jsdom"), validateAndCreateMetadata = require("../helpers/domain-metadata")["validateAndCreateMetadata"], ethers = require("ethers")["ethers"], rateLimit = require("express-rate-limit"), SHOW_FARHERO = !0, getCharacterSet = e => e.match(/^[a-zA-Z]+$/) ? "letter" : e.match(/^[0-9]+$/) ? "digit" : e.match(/^[a-zA-Z0-9]+$/) ? "alphanumeric" : e.match(/[\u{1F300}-\u{1F5FF}]/u) ? "emoji" : "mixed", background = async e => {
